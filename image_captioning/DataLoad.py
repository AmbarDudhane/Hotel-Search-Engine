...
def storeTotalDocuments(total):
    f = open(r'imp_files/totalDoc.txt', "w")
    f.write(str(total))
    f.close()

def storeIndex(index):
    f = open("imp_files/mainIndex.pkl", "wb")
    pickle.dump(index, f)
    f.close()
...
def computeTFIDF(mainIndex, total):
    ...
    f = open('imp_files/tfidfDataset.pkl', "wb")
    pickle.dump(tfidf, f)    # which can be used in searchIndex.py
    f.close()
    f1 = open('imp_files/idfDataset.pkl', "wb")
    pickle.dump(idfData, f1)
    f1.close()
    f2 = open('imp_files/tfDataset.pkl', "wb")
...
f1 = open(r"C:\Users\Ambar\OneDrive\Desktop\Image captioning\output_5k.pkl", "rb")
...
# print(len(dict1))
#
# print(dict1["000000150265.jpg"])
# print(dict1["000000308430.jpg"])
reviewDict = dict1
